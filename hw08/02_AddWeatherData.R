...
#Find geocodes for each of our cities using purrr::possibly and map_df

gcfunction <-possibly(~ geocode(., output = "latlon"),
                         NA_character_, quiet = TRUE)

outcity <- finalcitylist %$% 
  map_df(City, gcfunction)

finalcitylistgeo <-cbind(finalcitylist, outcity)
finalcitylistgeo <- finalcitylistgeo %>% 
  plyr::rename(replace = c("lat" = "latitude", "lon" = "longitude"))

#Use rnoaa package to look up nearest weather station id based on latitude and longtitude of city weather stations
